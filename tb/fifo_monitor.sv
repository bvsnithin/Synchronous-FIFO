class fifo_monitor #(parameter DATA_WIDTH = 8) extends uvm_monitor;
    ...
    // The "Radio Transmitter" to broadcast data
    uvm_analysis_port #(fifo_seq_item #(DATA_WIDTH)) mon_analysis_port;
    ...
    task run_phase(uvm_phase phase);
        // Temporary variable to hold captured data
            ...
            if (vif.rst_n) begin
                ...
                if (vif.wr_en || vif.rd_en) begin
                    ...
                    `uvm_info("MON", $sformatf("Saw Item: WR=%0d RD=%0d DataIn=%0h DataOut=%0h", 
                              item.wr_en, item.rd_en, item.din, item.dout), UVM_HIGH)
